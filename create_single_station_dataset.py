...
for yr in range(2000, 2019):
    ...
    for i in range(ss.shape[0]):
        ...
        distances = np.mean(np.sum((mag_data[:, :, :2] - ss_loc[None, None, :])**2, axis=2), axis=1)
    ...
    while i < n_positive_examples:
        ...
        if len(ss.iloc[random_date_index: random_date_index+Tfinal]) != 0:
            continue
        stations = np.arange(data.shape[0])
        np.random.shuffle(stations)
        mag_data = data[stations[:N_STATIONS], random_date_index - T0:random_date_index, 2:]
        if mag_data.shape != (N_STATIONS, T0, 3):
            continue
        if np.any(np.isnan(mag_data)):
            continue
